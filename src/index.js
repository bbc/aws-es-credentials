...
const resolveCredentials = async (options) => {
    if (options.useMetadataService === true) {
        ...
        return new AWS.Credentials(credentialsJSON.AccessKeyId, credentialsJSON.SecretAccessKey, credentialsJSON.Token)
    }
    ...
    return new AWS.Credentials(process.env.AWS_ACCESS_KEY_ID, process.env.AWS_SECRET_ACCESS_KEY, process.env.AWS_SESSION_TOKEN);
};
...
const mapProxy = (client) => {
    for (let fn in client) {
        const partialApplication = ((fn) =>  (...parameters) => clientProxyCaller(parameters, fn))(fn);
